...
export default function ReplayPage() {
  ...
  const lapOverlayVisible = !isMobile && isRace && lapAnalysisOpen && !!lapsResponse?.laps && !settings.showLeaderboard;
      ...
      <div className="flex-1 flex flex-col sm:flex-row min-h-0 overflow-y-auto sm:overflow-hidden pb-20 sm:pb-0">
        ...
        <div className={`sm:flex-1 min-w-0 ${!isMobile && showTelemetry && (selectedDrivers.length > 2 || settings.showAllPanels || rcPinned) ? `flex ${effectiveTelemetryPosition === "left" ? "flex-row" : "flex-col"} min-h-0` : "relative"}`}>
          ...
          {(!isMobile || mobileTrackOpen) && (
            <div
              className={`h-[42vh] sm:h-full relative ${!isMobile && showTelemetry && (selectedDrivers.length > 2 || settings.showAllPanels || rcPinned) ? "flex-1 min-w-0 min-h-0" : ""}`}
              style={lapOverlayVisible ? { paddingRight: 368 } : undefined}
            >
              {/* Flag badge */}
              {trackStatus !== "green" && (
              ...
              {/* Bottom-right action buttons - desktop */}
              {!isMobile && (
                <div className="absolute bottom-2 right-3 z-20 flex items-center gap-2">
                  ...
                  >
                    {settings.showLeaderboard ? "Hide Board" : "Show Board"}
                  </button>
                </div>
              )}

              {/* Lap Analysis fallback panel when leaderboard is hidden */}
              {!isMobile && isRace && lapAnalysisOpen && lapsResponse?.laps && !settings.showLeaderboard && (
                <div className="absolute top-2 right-2 bottom-12 w-[360px] border border-f1-border rounded bg-f1-card shadow-xl overflow-hidden z-20">
                  <LapAnalysisPanel
                    laps={lapsResponse.laps}
                    drivers={drivers}
                    currentLap={Math.max(0, (replay.frame?.lap || 0) - 1)}
                    replayYear={year}
                    replayRound={round}
                    sessionType={sessionType}
                    trackPoints={trackData?.track_points || []}
                    corners={trackData?.corners || []}
                    useImperial={settings.useImperial}
                    onClose={() => closePanel(() => setLapAnalysisOpen(false))}
                  />
                </div>
              )}
            </div>
